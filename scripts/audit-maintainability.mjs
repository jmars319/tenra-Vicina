...
function routeFromAppFile(file) {
  ...
  if (!relativeFile.startsWith("apps/webapp/src/app/")) return "";
  const routePath = relativeFile
    .replace(/^apps\/webapp\/src\/app\//u, "/")
    .replace(/\/page\.tsx$/u, "")
    .replace(/\/route\.ts$/u, "")
    .replace(/\/layout\.tsx$/u, "")
    .replace(/\/globals\.css$/u, "");
  if (!routePath || routePath === "/page.tsx") return "/";
  return routePath === "/" ? "/" : routePath.replace(/\/$/u, "") || "/";
}

function nextRoutes(allFiles) {
  return allFiles
    .filter((file) => /apps\/webapp\/src\/app\/.+\/(?:page\.tsx|route\.ts)$/u.test(relative(file)) || relative(file) === "apps/webapp/src/app/page.tsx")
    .map(routeFromAppFile)
    .filter(Boolean)
    .sort();
}

function suiteCatalogContracts() {
  const catalog = readJson("contracts/handoff-catalog.json", {});
  return {
    apps: (catalog.apps ?? []).map((app) => app.id).sort(),
    contracts: (catalog.contracts ?? []).map((contract) => contract.id).sort(),
    flows: (catalog.flows ?? []).map((flow) => flow.id).sort(),
    schema: catalog.schema ?? ""
  };
}

function handoffFixtureSchemas() {
  const fixtureRoot = path.join(root, "fixtures/handoffs");
  if (!fs.existsSync(fixtureRoot)) return [];
  return walkAll(fixtureRoot)
    .filter((file) => file.endsWith(".json"))
    .map((file) => {
      const payload = readJson(relative(file), {});
      return {
        path: relative(file),
...
function actualContracts(files) {
  ...
  const desktopFiles = allFiles.filter((file) => relative(file).startsWith("apps/desktopapp/src/"));
  return {
    ...
    handoffFixtures: handoffFixtureSchemas(),
    storageKeys: extractStringConstants(files, /(?:StorageKey|storageKey)\s*=\s*["']([^"']+)["']/gu),
    nextRoutes: nextRoutes(allFiles),
    suiteCatalog: suiteCatalogContracts(),
    tauriConfig: readJson("apps/desktopapp/src-tauri/tauri.conf.json", {}),
    desktopAppNames: extractStringConstants(desktopFiles, /appName\s*=\s*"([^"]+)"/gu),
    desktopMenuLabels: extractStringConstants(desktopFiles, /label:\s*"([^"]+)"/gu),
